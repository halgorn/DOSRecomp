...
std::expected<std::vector<std::byte>, file_error> virtual_file_system::read(std::uint16_t handle, std::size_t count) {
    ...
    found->second->read(reinterpret_cast<char*>(data.data()), static_cast<std::streamsize>(count));
    const auto read_count = found->second->gcount();
    if (found->second->bad()) return std::unexpected(file_error{"cannot read virtual DOS file"});
